...
The core engine of DURA is available as a standalone CLI tool.

## Installation

### Initial Setup (Required)
Before running your first analysis, DURA needs to install the Playwright browsers it uses to scrape GitHub release notes:
```bash
npx dura-kit setup
# If installed globally, just run: dura setup
```
...
```text
High-risk updates (2):
- express (prod) → confirmed breaking change
- eslint (dev) → major update with breaking changes

Medium-risk updates (4):
- @rollup/plugin-babel (dev) → major version gap
- chalk (dev) → major version gap
- jest (dev) → major version gap
- typescript (dev) → major version gap
...
High Priority Actions
  1. Review migration guides for 2 high-risk dependencies before updating
  2. Run full test suite after updating express and eslint

Medium Priority Actions
  1. Schedule updates for 4 medium-risk dependencies in next sprint
  2. Batch similar updates together for efficiency

Maintenance Actions
  1. Safe to run npm update or yarn upgrade for low-risk patches
  2. Run automated tests to verify compatibility

✔ Analysis complete

(Run "dura --help" for additional options)
```
...
### "Rate limit exceeded"
GitHub API rate limits may be reached. Wait a few minutes and try again, or use authenticated requests.
