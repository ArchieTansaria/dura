...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-background text-gray-900 dark:text-foreground font-sans selection:bg-accent/30 flex relative overflow-hidden">
      ...
      {/* Main Content Area */}
      <main className={cn(
        "flex-1 pt-[60px] min-h-screen relative z-10 flex flex-col bg-gray-50 dark:bg-[#050505] transition-all duration-300",
        isCollapsed ? "ml-[80px]" : "ml-[240px]"
      )}>
        
        {/* Subtle top ambient glow */}
        <div className="absolute top-0 left-1/4 w-1/2 h-64 bg-accent/10 dark:bg-accent/5 blur-[120px] rounded-full pointer-events-none -z-10" />
        ...
        <div className="p-8 lg:p-12 max-w-[1200px] w-full mx-auto flex-1 flex flex-col">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6 mb-10">
            <div>
              <h1 className="text-3xl font-semibold tracking-tight text-gray-900 dark:text-white mb-2">Repositories</h1>
              <p className="text-[15px] text-gray-600 dark:text-white/50 leading-relaxed font-medium">
          ...
          {installState === 'has-repos' && !isLoading && (
            <div className="mb-6 max-w-md relative group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-500 dark:text-white/30 group-focus-within:text-gray-900 dark:group-focus-within:text-white transition-colors" />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
                placeholder="Find a repository..."
                className="w-full bg-white dark:bg-white/[0.02] border border-gray-300 dark:border-white/5 hover:border-gray-400 dark:hover:border-white/10 focus:border-accent/40 dark:focus:border-white/20 focus:bg-gray-50 dark:focus:bg-white/[0.04] rounded-xl pl-10 pr-4 py-2.5 text-[14px] text-gray-900 dark:text-white placeholder:text-gray-500 dark:placeholder:text-white/30 outline-none transition-all duration-300 shadow-[0_1px_3px_rgba(0,0,0,0.1)] dark:shadow-sm font-medium"
              />
            </div>
          )}
