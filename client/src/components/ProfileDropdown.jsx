    ...
    <div className="relative" ref={dropdownRef}>
      <button 
        ...
        className={cn(
          ...
          isOpen ? "bg-white/10 border-white/10" : "border-transparent hover:bg-white/5 hover:border-white/5"
        )}
      >
        {avatarUrl ? (
      ...
      <div
        className={cn(
          "absolute right-0 top-full mt-2 w-64 bg-[#111] border border-white/10 rounded-2xl shadow-2xl z-50 transition-all duration-200 origin-top-right overflow-hidden",
      ...
      >
        ...
        <div className="p-3.5 flex items-center gap-3 border-b border-white/5">
          {avatarUrl ? (
            <img src={avatarUrl} alt={displayName} className="w-10 h-10 rounded-full shadow-sm ring-1 ring-white/10 object-cover shrink-0" />
          ) : (
          ...
          <div className="flex flex-col overflow-hidden">
            ...
            <span className="text-[12px] text-white/50 truncate font-light">{role}</span>
          </div>
        </div>
        ...
        <div className="p-1.5 flex flex-col">
          ...
          <div className="flex items-center justify-between px-2.5 py-2">
            <span className="text-[13px] text-white/80 font-medium">Theme</span>
            <div className="flex items-center gap-0.5 bg-white/5 p-0.5 rounded-lg border border-white/5">
              <button 
                onClick={() => setTheme('light')}
                className={cn("p-1.5 rounded-md transition-all", theme === 'light' ? "bg-white/10 text-white shadow-sm" : "text-white/40 hover:text-white/80")}
              >
                <Sun className="w-3.5 h-3.5" />
              </button>
              <button 
                onClick={() => setTheme('dark')}
                className={cn("p-1.5 rounded-md transition-all", theme === 'dark' ? "bg-white/10 text-white shadow-sm" : "text-white/40 hover:text-white/80")}
              >
                <Moon className="w-3.5 h-3.5" />
              </button>
              <button 
                onClick={() => setTheme('system')}
                className={cn("p-1.5 rounded-md transition-all", theme === 'system' ? "bg-white/10 text-white shadow-sm" : "text-white/40 hover:text-white/80")}
              >
          ...
          {/* Account Settings */}
          <button className="flex items-center gap-2.5 w-full px-2.5 py-2 text-left text-[13px] text-white/80 hover:bg-white/5 hover:text-white rounded-lg transition-colors group">
          ...
          {/* Logout Action */}
          <a href={`${import.meta.env.VITE_BACKEND_URL}/logout`} className="flex items-center gap-2.5 w-full px-2.5 py-2 text-left text-[13px] text-white/80 hover:bg-red-500/10 hover:text-red-400 rounded-lg transition-colors group">
