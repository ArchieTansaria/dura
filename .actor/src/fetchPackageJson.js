 ...
 * @param {string} [branch] - Optional branch name (defaults to trying main, master, then default branch)
 * @returns {Promise<Object>} - Parsed package.json object
 */
async function fetchPackageJson(repoUrl, branch = null) {
  const { owner, repo } = parseGitHubUrl(repoUrl);

  // If branch is specified, try it first
  if (branch) {
    const pkg = await tryBranch(owner, repo, branch);
    if (pkg) return pkg;
  }
